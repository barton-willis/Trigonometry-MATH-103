...
``everything.'' For equations from algebra, ``everything'' generally means
...
\quad A more relaxed notion of an identity is an equation whose
solution set is the same as its natural domain. For that meaning, \(\displaystyle \frac{x}{x} = 1\)
is an identity because both its solution and its natural domain are \(\reals_{\neq 0}\).

\item \textbf{Question} To prove that an equation is an identity, I was
 told that to ``only work with one side at a time.'' Why is that?

 \textbf{Answer} The requirement to ``only work with one side at a time''
 is an effort to disallow doing things like this: Suppose you were asked To
 show that \(x = 1\) is an identity. Working with both sides, you might do
\begin{align*}
    \left [ x = 1 \right] &\implies [0 x = 0 \times 1], & \mbox {(multiply both sides by zero)},\\
                          &\equiv [0 x = 0], & \mbox{ (algebra)} \\
                          &\equiv [ 0 = 0],  & \mbox{ (algebra)} \\
                          & \mbox{True} & \mbox{ (number equality)}
\end{align*}
This work is perfect, but we need to understand what we just proved.
We've shown that if \(x=1\) then \(0 = 0\). This is a conditional 
statement with a conclusion that is manifestly true. The rules of
Boolean logic say that a conditional statement with a true conclusion
has a truth value of true regardless of the truth value of the hypothisi.
